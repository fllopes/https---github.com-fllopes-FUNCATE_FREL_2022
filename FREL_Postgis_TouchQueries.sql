...
-- Seleciona geometrias que se tocam, mas que tem classes diferentes
...
-- Seleciona geometrias com sobreposição no mesmo layer
...
-- Seleciona geometrias com sobreposição no mesmo layer com polígonos da mesma classe
-- a.geom && b.geom: retorna true se houver intersecção entre o box das entradas. Tira proveito do índice espacial, ganhando eficiência na execução.
-- ST_Relate(a.geom, b.geom, '2********'): condição de que os polígonos devem satisfazer a condição '2********' (padrão DE-9IM), isto é, a intersecção dos polígonos deve formar um polígono.
-- Resultado: 
...
-- Seleciona as geometrias iguais. Não são polígonos iguais, mas sim geometrias, portanto as classes podem ser diferentes.
